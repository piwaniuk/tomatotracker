...
SongReaderCurrent::SongReader(std::ifstream&& file, std::unique_ptr<SongHeader> header) :
...
static Parameters1Osc* take1OscParams(uint8_t* buffer) {
  ...
  params->ampAtt = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_A_OFF);
  params->ampDec = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_D_OFF);
  params->ampSus = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_S_OFF);
  params->ampRel = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_R_OFF);
  params->filter = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_CUTOFF_OFF);
  params->res = fromBuffer<uint16_t>(buffer + Fmt::PARAMS_1OSC_RES_OFF);
  return params;
}
...
Song* SongReaderCurrent::loadSong() {
